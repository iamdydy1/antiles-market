...
export default async function SearchPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [listings, territories, categories] = await Promise.all([
    prisma.listing.findMany({ where, orderBy, take: 60, include: { territory: { select: { name: true } }, category: { select: { name: true, icon: true } }, images: { orderBy: { position: "asc" }, take: 1 } } }),
  ...
  return (
    <main className="searchPage">
      ...
      <form className="filtersBar" action="/recherche" method="get">
        ...
        <label><span>Catégorie</span><select name="category" defaultValue={categoryId}><option value="">Toutes les catégories</option>{categories.map((c) => <option value={c.id} key={c.id}>{c.icon} {c.name}</option>)}</select></label>
        <label><span>Prix min.</span><input name="min" type="number" min="0" defaultValue={min ?? ""} /></label>
        <label><span>Prix max.</span><input name="max" type="number" min="0" defaultValue={max ?? ""} /></label>
        <label><span>Trier</span><select name="sort" defaultValue={sort}><option value="recent">Plus récentes</option><option value="price_asc">Prix croissant</option><option value="price_desc">Prix décroissant</option></select></label>
        <button className="primaryButton" type="submit">Rechercher</button>
      </form>

      <section className="searchResultsShell"><div className="resultsHeading"><div><h2>{listings.length} annonce{listings.length !== 1 ? "s" : ""}</h2><p>{q ? `Résultats pour « ${q} »` : "Toutes les annonces disponibles"}</p></div><a href="/recherche">Réinitialiser les filtres</a></div>
        {listings.length === 0 ? <div className="emptyStateCard"><span>🔎</span><h2>Aucune annonce trouvée</h2><p>Essayez d'élargir vos critères de recherche.</p></div> : <div className="resultsGrid">{listings.map((listing) => <a className="resultCard" href={`/annonces/${listing.slug}`} key={listing.id}><div className="resultImage">{listing.images[0] ? <img src={listing.images[0].url} alt={listing.title} /> : <span>📷</span>}</div><div className="resultBody"><small>{listing.category.icon} {listing.category.name}</small><h2>{listing.title}</h2><strong>{listing.price ? new Intl.NumberFormat("fr-FR", { style: "currency", currency: listing.currency }).format(Number(listing.price)) : "Prix sur demande"}</strong><span>📍 {listing.territory.name}</span></div></a>)}</div>}
      </section>
    </main>
  );
}
