...
export default async function ListingDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const listing = await prisma.listing.findUnique({
    where: { slug },
    include: {
      seller: { select: { id: true, displayName: true, createdAt: true, accountType: true, companyName: true } },
      territory: { select: { name: true } },
      location: { select: { name: true } },
      category: { select: { id: true, name: true, slug: true, icon: true, parent: { select: { icon: true, slug: true } } } },
  ...
  const eventUrl = listing.eventUrl && /^https?:\/\//i.test(listing.eventUrl) ? listing.eventUrl : null;
