...
export async function POST(request: Request) {
  try {
    ...
    const numericPrice = Number(input.price.replace(",", "."));
    if (!Number.isFinite(numericPrice) || numericPrice < 0 || numericPrice > 999999999) {
      return NextResponse.json({ error: "Prix invalide." }, { status: 400 });
    }

    const [territory, category] = await Promise.all([
    ...
    if (!territory || !category) return NextResponse.json({ error: "Île ou catégorie invalide." }, { status: 400 });
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: "Vérifiez les informations de votre annonce." }, { status: 400 });
    }
